...
app.post('/api/tasks', async (req, res) => {
  try {
    ...
    if (!user || user.length === 0) {
      return res.status(400).json({ detail: 'Usuario nao encontrado' })
    }
...
function gerarRespostaSimples(tarefasPendentes, tarefasVencidas, eventosHoje, nomeUsuario, perguntaUsuario) {
  ...
  if (tarefasVencidas.length > 0) {
    ...
    tarefasVencidas.slice(0, 3).forEach(t => { msg += ' - ' + t.title + '\n' })
  ...
  if (eventosHoje.length > 0) {
    msg += '\nEventos de hoje:\n'
    eventosHoje.forEach(e => { msg += ' - ' + e.title + (e.time ? ' as ' + e.time : '') + '\n' })
  }
  ...
  if (perguntaUsuario) {
    ...
    if (p.includes('prioridad') || p.includes('importante')) {
      ...
      msg += '\nDica: Comece pelo mais dificil primeiro!\n'
    }
  } else {
